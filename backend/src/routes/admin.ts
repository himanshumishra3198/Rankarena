...
// Bulk import: create bank questions and link them all to this mock.
// `subject` may be omitted per row — it defaults to the mock's own section.
const bulkMockQuestionSchema = z.array(
  questionSchema.extend({
    subject: z.enum(["QUANT", "REASONING", "ENGLISH", "GK"]).optional(),
    marks: z.number().min(0.1).default(2),
    negativeMarks: z.number().min(0).default(0.5),
  })
).min(1);

router.post("/mocks/:id/questions/bulk", async (req: AuthRequest, res: Response) => {
  const mockTestId = req.params.id as string;
  const mock = await prisma.mockTest.findUnique({
    where: { id: mockTestId },
    select: { subject: true },
  });
  if (!mock) { res.status(404).json({ error: "Mock test not found." }); return; }

  const parsed = bulkMockQuestionSchema.safeParse(req.body);
  if (!parsed.success) { res.status(400).json({ error: parsed.error.issues }); return; }

  // Fill in the mock's section for any row that didn't specify one.
  const rows = parsed.data.map((q) => ({ ...q, subject: q.subject ?? mock.subject }));
  ...
  // Skip exact duplicates: against the DB and within the batch itself.
  const fingerprints = rows.map((q) => fingerprintOf(q));
  const existing = await prisma.question.findMany({
    where: { fingerprint: { in: fingerprints } },
    select: { fingerprint: true },
  });
    ...
    toCreate.map(({ marks, negativeMarks, ...qData }) =>
      prisma.question.create({
        data: {
          ...
          fingerprint: fingerprintOf(qData),
          mockTestQuestions: {
            create: { mockTestId, displayOrder: nextOrder++, marks, negativeMarks },
          },
        } as Prisma.QuestionCreateInput,
      })
    )
